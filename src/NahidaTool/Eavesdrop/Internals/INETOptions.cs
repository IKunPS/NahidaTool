...
namespace NahidaTool.Eavesdrop.Internals
{
    public static class INETOptions
    {
        ...
        private static string _originalHTTPAddress;
        private static string _originalHTTPSAddress;
        private static bool _originalProxyEnabled;
        private static bool _originalIgnoringLocalTraffic;
        private static readonly List<string> _originalOverrides;
        ...
        static INETOptions()
        {
            ...
            _proxyKey = Registry.CurrentUser.OpenSubKey(@"Software\Microsoft\Windows\CurrentVersion\Internet Settings", true)
                ?? throw new InvalidOperationException("无法打开当前用户 Internet Settings 注册表项");

            Overrides = new List<string>();
            Load();
            _originalHTTPAddress = HTTPAddress;
            _originalHTTPSAddress = HTTPSAddress;
            _originalProxyEnabled = IsProxyEnabled;
            _originalIgnoringLocalTraffic = IsIgnoringLocalTraffic;
            _originalOverrides = new List<string>(Overrides);
        }
        ...
        public static void Save()
        {
            lock (_stateLock)
            {
                ...
                bool saved = NativeMethods.InternetSetOption(IntPtr.Zero, 75, iNetPackagePtr, _iNetPackageSize);
                int error = saved ? 0 : Marshal.GetLastWin32Error();
                if (!saved)
                {
        ...
        public static void RestoreOriginal()
        {
            lock (_stateLock)
            {
                HTTPAddress = _originalHTTPAddress;
                HTTPSAddress = _originalHTTPSAddress;
                IsProxyEnabled = _originalProxyEnabled;
                IsIgnoringLocalTraffic = _originalIgnoringLocalTraffic;
                Overrides.Clear();
                Overrides.AddRange(_originalOverrides);
                Save();
            }
        }
